...
export async function POST() {
  ...
  if (!mail.ok) {
    return NextResponse.json({
      error: "E-posta gönderilemedi. Resend hesabına kayıtlı e-posta ile giriş yap veya domain doğrula.",
    }, { status: 500 });
  }
  ...
  const body: Record<string, unknown> = {
    success: true,
    message: mail.dev || isDev
      ? "Test modunda mail sadece Resend hesabına gider. Doğrulama linki aşağıda — tıkla."
      : "Doğrulama bağlantısı e-posta adresinize gönderildi.",
  };
  if (isDev && mail.verifyUrl) {
    body.devVerifyUrl = mail.verifyUrl;
  } else if (mail.dev && "verifyUrl" in mail) {
    body.devVerifyUrl = mail.verifyUrl;
  }
