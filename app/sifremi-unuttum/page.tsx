    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#F7F7F5", padding: "1rem" }}>
      <div style={{ background: "#fff", borderRadius: 18, border: "0.5px solid #E8E8E5", padding: "2.5rem", width: "100%", maxWidth: 420, boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
        ...
        <Link href="/" style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 800, fontSize: 22, textDecoration: "none", display: "inline-flex", alignItems: "baseline", marginBottom: "2rem" }}>
          ...
          <span style={{ color: "var(--brand)" }}>ra</span>
          <span style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--brand)", display: "inline-block", marginLeft: 1, marginBottom: 8 }} />
        </Link>
        ...
        {step === "email" && (
          <>
            ...
            {error && <div style={{ background: "var(--brand-soft)", color: "#dc2626", padding: "10px 14px", borderRadius: 9, fontSize: 13.5, marginBottom: "1rem", border: "0.5px solid var(--brand-border)" }}>⚠️ {error}</div>}
            <form onSubmit={handleEmail}>
              ...
              />
              <button type="submit" disabled={loading} style={{ width: "100%", padding: "12px", background: loading ? "#ccc" : "var(--brand)", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
        ...
        {step === "reset" && (
          <>
            <div style={{ width: 48, height: 48, borderRadius: "50%", background: "var(--brand-soft)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "1rem" }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--brand)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
            </div>
            ...
            <form onSubmit={handleReset}>
              ...
              <button type="submit" disabled={loading} style={{ width: "100%", padding: "12px", background: loading ? "#ccc" : "var(--brand)", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                {loading ? (
        ...
        {step === "done" && (
          <div style={{ textAlign: "center" }}>
            ...
            <Link href="/giris" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "12px 28px", background: "var(--brand)", color: "#fff", borderRadius: 10, textDecoration: "none", fontWeight: 700, fontSize: 15 }}>
              Giriş yap →
            </Link>
          </div>
        )}
        ...
        {step !== "done" && (
          <p style={{ textAlign: "center", fontSize: 13.5, color: "#888", marginTop: "1.5rem" }}>
            <Link href="/giris" style={{ color: "var(--brand)", textDecoration: "none", fontWeight: 500 }}>← Giriş sayfasına dön</Link>
          </p>
        )}
      </div>
